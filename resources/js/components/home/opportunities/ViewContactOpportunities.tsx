...
import { ArrowDown, ArrowUp, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Eye, Filter, Search, Trash, X } from 'lucide-react';
...
const ViewContactOpportunities: React.FC<ViewOpportunitiesProps> = ({ setEdit, setActiveTab, activeTab, contact_id }) => {
    ...
    }, [activeTab, page, searchQuery, searchField, filters]);
    ...
    const handleFilterChange = (key: keyof typeof filters, value: string | { start: string; end: string } | OpportunityStage | any) => {
        setFilters((prev) => ({
            ...prev,
            [key]: value || undefined,
        }));
    ...
    const activeFilters = useMemo(() => {
        ...
        if (filters.assistant_clerk) {
            ...
            filtersList.push({
                ...
                value: user?.name || filters.assistant_clerk,
            });
        }
    ...
    return (
        <div className="mt-4 px-2 sm:px-4">
                        ...
                        {isFilterOpen && (
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                ...
                                <div>
                                    ...
                                    <select
                                        value={filters.dateFilter?.field || ''}
                                        onChange={(e) =>
                                ...
                                {filters.dateFilter?.field && (
                                    <div>
                                        ...
                                        <select
                                            value={filters.dateFilter?.range || ''}
                                            onChange={(e) =>
                                                handleFilterChange('dateFilter', {
                                                    ...filters.dateFilter,
                                ...
                                {filters.dateFilter?.range === 'custom' && filters.dateFilter?.field && (
                                    <div>
            ...
            <div className="mb-4 text-sm text-gray-600">{isFetching ? <span></span> : <span>Found {totalCount} matching opportunities</span>}</div>
                ...
                <>
                    {isMobileView ? (
                        <div className="space-y-4">
                            {sortedOpportunities.map((opportunity: Opportunity) => (
                                <div key={opportunity.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                                    <div>
                                                ...
                                                <Autocomplete
                                                    ...
                                                    renderInput={(params) => (
                                                        <TextField
                                                            ...
                                                            variant="outlined"
                                                            className="w-full"
                                                            InputProps={{ ...params.InputProps, className: 'rounded-lg text-sm' }}
                                                        />
                                                    )}
                                    ...
                                    <span className={`rounded-full px-2 py-1 text-xs font-semibold ${getProbabilityStyle(opportunity.probability)}`}>
                                    ...
                                    <div className="mb-3 grid grid-cols-2 gap-2 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <p>
                                                {opportunity.last_update?.updated_at ? formatTimestampToDate(opportunity.last_update.updated_at) : ''}
                                                {user?.role === 'admin' && opportunity.last_update?.updated_by && (
                                                    <p className="text-sm text-gray-500">By {opportunity.last_update?.updated_by?.name}</p>
                                                )}
                                            </p>
                                        </div>
                                        <div>
                                            <p className="text-gray-500">Owner</p>
                                            <p>{opportunity.assistant_clerk.name}</p>
                                        </div>
                                    </div>
                                    ...
                                    <tbody className="divide-y divide-gray-200 bg-white">
                                        {sortedOpportunities.map((opportunity: Opportunity) => (
                                            <tr key={opportunity.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-4 text-sm font-medium whitespace-nowrap text-gray-900">{opportunity.name}</td>
                                                <td className="px-4 py-4 text-sm whitespace-nowrap text-gray-500">
                                                    {opportunity.contact.institution ||
                    ...
                    {totalPages > 1 && (
                        <div className="mt-6 flex items-center justify-between px-2 sm:px-0">
                            <div className="flex flex-1 justify-between sm:hidden">
                                <button
                                    ...
                                    className={`relative inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium ${
                                        page === 0 ? 'cursor-not-allowed bg-gray-100 text-gray-400' : 'bg-white text-gray-700 hover:bg-gray-50'
                                    }`}
                                >
                                    Previous
                                </button>
                            ...
                            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                <div>
                                    <p className="text-sm text-gray-700">
                                        ...
                                        <span className="font-medium">{Math.min((page + 1) * ITEMS_PER_PAGE, totalCount)}</span> of{' '}
                                    ...
                                    <nav className="relative z-0 inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                                        <button
                                            ...
                                            className={`relative inline-flex items-center rounded-l-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium ${
                                                page === 0 ? 'cursor-not-allowed text-gray-300' : 'text-gray-500 hover:bg-gray-50'
                                            }`}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`relative inline-flex items-center rounded-r-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium ${
                                                page === totalPages - 1 ? 'cursor-not-allowed text-gray-300' : 'text-gray-500 hover:bg-gray-50'
                                            }`}
                                        >
...
export default ViewContactOpportunities;
