...
const FileUploads: React.FC = () => {
    ...
    useEffect(() => {
        const fetchFiles = async () => {
            setIsFetching(true);
            try {
                const response = await axios.get(`/api/file-uploads${searchQuery ? `?search=${encodeURIComponent(searchQuery)}` : ''}`);
        ...
        if (files.length > 0) {
            for (const file of files) {
                ...
                if (
                    ...
                    toast.error('Only PDF, DOC, DOCX, JPEG, PNG, or GIF files are allowed');
                    return;
                }
            }
            setFormData((prev) => ({
            ...
            {(showCreateModal || showEditModal) && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white p-6">
                        <h3 className="mb-4 text-lg font-semibold">{showEditModal ? 'Edit File' : 'Upload New Files'}</h3>
                        <div className="space-y-6">
                            <div>
                                <label className="mb-2 block text-sm font-medium text-gray-700">
                                    Files * <span className="text-xs text-gray-500">(Max 50MB, PDF/DOC/DOCX/JPEG/PNG/GIF)</span>
                                </label>
                                <div className="flex items-center space-x-4">
                                    ...
                                    <label className="cursor-pointer rounded-md border bg-gray-100 px-4 py-2 transition-colors hover:bg-gray-200">
                                        {formData.files.length > 0 ? 'Change Files' : 'Select Files'}
                                        <input
                                            type="file"
                                            className="hidden"
                                ...
                                {formData.files.length > 0 && (
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-600">Selected Files ({formData.files.length}):</p>
                        ...
                        <table className="min-w-full table-auto text-left text-sm text-gray-500">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="cursor-pointer px-4 py-2 hover:bg-gray-200" onClick={() => handleSort('file_size')}>
                                        <div className="flex items-center">
                                            Size{' '}
                                            {sortField === 'file_size' &&
                                                (sortDirection === 'desc' ? <ArrowDown size={16} /> : <ArrowUp size={16} />)}
                            ...
                            <tbody>
                                {sortedFiles?.map((file) => (
                                    <tr key={file.id} className="border-b hover:bg-gray-100">
                                        ...
                                        <td className="px-4 py-4">{formatFileSize(file.file_size)}</td>
                                        <td className="px-4 py-4">{file.file_type}</td>
                                        <td className="px-4 py-4">{file.category || '-'}</td>
                                ...
                                {expandedFile === file.id && (
                                    <div className="mt-3 space-y-2">
                                        <div className="flex flex-col">
                                            ...
                                            {file.file_type.startsWith('image/') ? (
                                                <img src={file.file_path} alt={file.file_name} className="mt-1 h-32 w-full rounded object-cover" />
                                            ) : (
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-sm">{file.category || '-'}</span>
                                        </div>
                                        ...
                                        <div className="flex gap-2 pt-2">
                                            <button className="flex-1 rounded bg-blue-500 py-1 text-sm text-white" onClick={() => handleEdit(file)}>
                                                Edit File
                                            </button>
                                            <button
