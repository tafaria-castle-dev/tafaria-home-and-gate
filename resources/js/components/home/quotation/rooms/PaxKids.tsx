...
export default function PaxKids() {
  const [kids, setKids] = useState<{ id: number; age: number }[]>([]);

  const addKid = () => {
    setKids((prev) => [...prev, { id: prev.length + 1, age: 12 }]); // Default age is 12
  };
  ...
  const updateAge = (id: number, age: number) => {
    setKids((prev) =>
      prev.map((kid) => (kid.id === id ? { ...kid, age } : kid))
    );
  };
...
<div className="bg-white p-4 rounded-lg w-full outline flex items-center gap-6">
  ...
  <div className="flex items-center bg-white rounded-lg w-64">
    ...
    <div className="flex items-center gap-2">
      ...
      <span className="text-lg font-semibold text-gray-800">{kids.length}</span>
